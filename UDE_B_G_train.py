...
def NN_G(input, neurons_HiddenLayer, wH, bH, wOut, bOut):
    """
    Neural network to predict the growth rate (G)

    :param input: normalized inputs of the neural network (concentration and temperature)
    :param neurons_HiddenLayer: number of neurons in the hidden layer
    :param wH: weights matrix of the neurons between the input and the hidden layers
    :param bH: biases vector of the hidden layer
    :param wOut: weights matrix of the neurons between the hidden and the output layers
    :param bOut: biases vector of the output layer
    :return: predicted growth rate (G)
    """
...
def modelagem_PB(entrada, t, T, wH_B, bH_B, wOut_B, bOut_B, wH_G, bH_G, wOut_G, bOut_G):
    """
    ...
    :param wH_G: weights matrix of the neurons between the input and the hidden layers for
    the neural network to predict G
    :param bH_G: bH_B: biases vector of the hidden layer for the neural network to predict G
    :param wOut_G:weights matrix of the neurons between the hidden and the output layers for
    the neural network to predict G
    :param bOut_G: bOut_B: biases vector of the output layer for the neural network to predict G
    ...
    input_G = np.array([C_norm, T_norm])
...
def integrator_PB(y0, t, temperature_range, wH_B, bH_B, wOut_B, bOut_B, wH_G, bH_G, wOut_G, bOut_G):
    """
    ...
    for i in range(len(t) - 1):
        ...
        ts = [t[i], t[i + 1]]
        y = odeint(modelagem_PB, y0, ts, args=(temperature_range[i + 1], wH_B, bH_B, wOut_B, bOut_B,
                                               wH_G, bH_G, wOut_G, bOut_G), atol=1e-8, rtol=1e-8)
...
def f_obj(parameters_NN, number_inputs_B, neurons_HidenLayer_B,
          number_outputs_B, number_inputs_G, neurons_HidenLayer_G, number_outputs_G):
    """
    ...
    :param number_inputs_G: number of inputs of the neural network to predict G
    :param neurons_HidenLayer_G: number of hidden layers of the neural network to predict G
    :param number_outputs_G: number of inputs of the neural network to predict G
    ...
    wH_G = np.zeros([number_inputs_G, neurons_HidenLayer_G])
    bH_G = np.zeros(neurons_HidenLayer_G)
    wOut_G = np.zeros([neurons_HidenLayer_G, number_outputs_G])
    bOut_G = np.zeros(number_outputs_G)
    ...
    for m in range(number_inputs_G):
        for n in range(neurons_HidenLayer_G):
            wH_G[m][n] = parameters_NN[count3]

            count3 += 1

    for m in range(neurons_HidenLayer_G):
        bH_G[m] = parameters_NN[count3]

        count3 += 1

    for m in range(neurons_HidenLayer_G):
        for n in range(number_outputs_G):
            wOut_G[m][n] = parameters_NN[count3]

            count3 += 1

    for m in range(number_outputs_G):
        bOut_G[m] = parameters_NN[count3]
    ...
    for m in range(batches):
        prediction = integrator_PB(y0[m], time[m], Temperature[m], wH_B, bH_B, wOut_B, bOut_B, wH_G, bH_G, wOut_G,
                                   bOut_G)
...
wH_G0 = 1e-4 * np.ones(number_inputs_G * neurons_HidenLayer_G)
biasH_G0 = 1e-6 * np.ones(neurons_HidenLayer_G)
wOut_G0 = 1e-4 * np.ones(neurons_HidenLayer_G * number_outputs_G)
biasOut_G0 = 1e-6 * np.ones(number_outputs_G)

# Creating a vector containing all initial guesses
terms = len(wH_B0) + len(biasH_B0) + len(wOut_B0) + len(biasOut_B0) + \
        len(wH_G0) + len(biasH_G0) + len(wOut_G0) + len(biasOut_G0)
...
for i in range(len(wH_G0)):
    x0[count2] = wH_G0[i]
    count2 += 1

for i in range(len(biasH_G0)):
    x0[count2] = biasH_G0[i]
    count2 += 1

for i in range(len(wOut_G0)):
    x0[count2] = wOut_G0[i]
    count2 += 1

for i in range(len(biasOut_G0)):
    x0[count2] = biasOut_G0[i]
    count2 += 1
...
sol = minimize(f_obj, x0, method='SLSQP', args=(number_inputs_B,
                                                ...
                                                number_inputs_G, neurons_HidenLayer_G,
                                                number_outputs_G,), options={'maxiter': 100, 'ftol': 1e-08})
print(sol)
print(f'x ótimo = {sol.x}')
print(
    f'f(x ótimo) = {f_obj(sol.x, number_inputs_B, neurons_HidenLayer_B, number_outputs_B, number_inputs_G, neurons_HidenLayer_G, number_outputs_G)}')

solution = np.array([f_obj(sol.x, number_inputs_B, neurons_HidenLayer_B, number_outputs_B, number_inputs_G,
                           neurons_HidenLayer_G, number_outputs_G)])

header1 = 'x'
header2 = 'fobj'

np.savetxt("x_op_UDE_B_G_7exps.csv", sol.x, delimiter=',', header=header1)
np.savetxt("f_obj_UDE_B_G_7exps.csv", solution, delimiter=',', header=header2)
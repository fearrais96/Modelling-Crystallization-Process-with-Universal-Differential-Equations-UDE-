...
def modelagem_PB(entrada, t, T, wH_G, bH_G, wOut_G, bOut_G):
    """
    ...
    B = kb * np.exp(-Eab_R / (T + 273.15)) * (S ** 2) ** (cb / 2) * ((r_3) ** 2) ** (beta / 2)*(mu0**(beta-1))
...
def integrator_PB(y0, t, temperature_range, wH_G, bH_G, wOut_G, bOut_G):
    """
    ...
    for i in range(len(t) - 1):
        ...
        ts = [t[i], t[i + 1]]
        y = odeint(modelagem_PB, y0, ts, args=(temperature_range[i + 1],wH_G, bH_G, wOut_G, bOut_G),
                   atol=1e-8, rtol=1e-8)
...
def f_obj(parameters_NN, number_inputs_G, neurons_HidenLayer_G, number_outputs_G):
    """
    ...
    for m in range(batches):
        prediction = integrator_PB(y0[m], time[m], Temperature[m], wH_G, bH_G, wOut_G, bOut_G)
...
terms = len(wH_G0) + len(biasH_G0) + len(wOut_G0) + len(biasOut_G0)
...
# x0 = pd.read_csv("x_op_UDE_G_7exps.csv")

# Sovilg the optimization problem
sol = minimize(f_obj, x0, method='SLSQP', args=(number_inputs_G, neurons_HidenLayer_G,number_outputs_G,),
               options={'maxiter': 5, 'ftol': 1e-08})

print(sol)
print(f'x ótimo = {sol.x}')
print(
    f'f(x ótimo) = {f_obj(sol.x, number_inputs_G, neurons_HidenLayer_G, number_outputs_G)}')

solution = np.array([f_obj(sol.x, number_inputs_G, neurons_HidenLayer_G, number_outputs_G)])

header1 = 'x'
header2 = 'fobj'

np.savetxt("x_op_UDE_G_7exps.csv", sol.x, delimiter=',', header=header1)
np.savetxt("f_obj_UDE_G_7exps.csv", solution, delimiter=',', header=header2)